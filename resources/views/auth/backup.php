...
.auth-card {
  ...
  border:1px solid var(--border-lit); border-radius:20px; padding:40px 40px 32px;
...
.brand-icon { width:36px; height:36px; background:var(--accent); border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:1.1rem; }
...
.prog-wrap { display:none; gap:6px; margin-bottom:28px; }
.prog-wrap.show { display:flex; }
.prog-seg { height:3px; flex:1; border-radius:100px; background:var(--border-lit); transition:background .4s; }
.prog-seg.done   { background:var(--green); }
.prog-seg.active { background:var(--accent); }

/* Step label */
.step-label { display:flex; align-items:center; gap:8px; font-family:var(--mono); font-size:.68rem; color:var(--text-muted); letter-spacing:.05em; text-transform:uppercase; margin-bottom:8px; }
.step-badge { width:20px; height:20px; border-radius:50%; background:var(--border-lit); color:var(--text-dim); font-size:.65rem; font-weight:600; display:flex; align-items:center; justify-content:center; }
...
.card-sub   { font-size:.82rem; color:var(--text-muted); margin-bottom:24px; }
.card-sub a { color:var(--accent); text-decoration:none; font-weight:500; }

/* Alert */
.alert-box { display:none; padding:10px 14px; border-radius:10px; font-size:.81rem; margin-bottom:16px; align-items:center; gap:8px; }
...
.field-label { display:block; font-family:var(--mono); font-size:.67rem; font-weight:500; letter-spacing:.1em; text-transform:uppercase; color:var(--text-muted); margin-bottom:6px; }
.input-wrap { position:relative; }
.input-wrap input { width:100%; background:var(--surface-2); border:1px solid var(--border-lit); border-radius:10px; padding:11px 40px 11px 40px; font-family:var(--sans); font-size:.88rem; color:var(--text); outline:none; transition:border-color .2s, box-shadow .2s; }
.input-wrap input:focus { border-color:var(--accent); box-shadow:0 0 0 3px var(--accent-glow); }
.input-wrap input::placeholder { color:var(--text-dim); }
.input-icon  { position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--text-muted); pointer-events:none; font-size:.88rem; }
.toggle-eye  { position:absolute; right:12px; top:50%; transform:translateY(-50%); color:var(--text-muted); cursor:pointer; font-size:.88rem; }
.toggle-eye:hover { color:var(--text); }
.field-error { font-size:.73rem; color:#fca5a5; margin-top:4px; }

/* Strength */
.strength-wrap { height:3px; background:var(--border-lit); border-radius:100px; margin-top:7px; overflow:hidden; }
.strength-fill  { height:100%; border-radius:100px; width:0; transition:width .4s, background .4s; }
.req-row { display:flex; flex-wrap:wrap; gap:6px; margin-top:7px; }
.req-chip { font-size:.72rem; color:var(--text-dim); display:flex; align-items:center; gap:4px; transition:color .3s; }
.req-chip.ok { color:var(--green); }
.req-dot { width:5px; height:5px; border-radius:50%; background:currentColor; flex-shrink:0; }

/* Terms */
.terms-row { display:flex; align-items:flex-start; gap:9px; font-size:.8rem; color:var(--text-muted); margin-bottom:4px; }
.terms-row input[type="checkbox"] { margin-top:2px; accent-color:var(--accent); flex-shrink:0; }
...
.twofa-row:hover { border-color:var(--accent); }
.twofa-row.on { border-color:#065f46; background:#052e1630; }
.twofa-row-info { flex:1; }
.twofa-row-title {
  font-size:.875rem; font-weight:600; color:var(--text);
  display:flex; align-items:center; gap:8px; margin-bottom:3px;
}
.status-pill {
  font-family:var(--mono); font-size:.6rem; font-weight:600; letter-spacing:.05em;
  padding:2px 8px; border-radius:100px; transition:all .3s;
}
.pill-off { background:var(--border-lit); color:var(--text-dim); border:1px solid var(--border); }
.pill-on  { background:var(--green-dim);  color:var(--green);    border:1px solid #065f46; }
.twofa-row-desc { font-size:.75rem; color:var(--text-muted); line-height:1.4; }
/* Toggle switch */
.tog { position:relative; width:40px; height:22px; flex-shrink:0; }
.tog input { opacity:0; width:0; height:0; position:absolute; }
.tog-track {
  position:absolute; inset:0; border-radius:100px;
  background:var(--border-lit); cursor:pointer; transition:background .3s;
}
.tog-track::before {
...
.tog input:checked ~ .tog-track::before { transform:translateX(18px); background:#fff; }

/* QR */
.qr-frame {
  width:172px; height:172px; border:2px dashed var(--border-lit); border-radius:14px;
  margin:0 auto 16px; display:flex; align-items:center; justify-content:center;
  background:var(--surface-2); overflow:hidden;
}
.qr-frame img { width:100%; height:100%; object-fit:contain; border-radius:12px; }
.qr-ph { font-size:.78rem; color:var(--text-dim); text-align:center; line-height:1.6; }
.secret-copy-wrap { position:relative; margin-bottom:6px; }
.secret-copy-wrap input {
  ...
  font-family:var(--mono); font-size:.85rem; letter-spacing:.08em; color:var(--accent);
  outline:none; cursor:pointer;
}
.copy-btn { position:absolute; right:12px; top:50%; transform:translateY(-50%); cursor:pointer; font-size:.95rem; }
.copy-ok  { font-size:.72rem; color:var(--green); margin-top:5px; display:none; }
...
.otp-box:focus { border-color:var(--accent); box-shadow:0 0 0 3px var(--accent-glow); }
.otp-box.filled { border-color:#065f46; background:#052e1630; }
.timer-hint { font-family:var(--mono); font-size:.72rem; color:var(--text-dim); text-align:right; margin-top:8px; }
.timer-hint .cd { color:var(--accent); }
.back-link { text-align:center; margin-top:12px; }
.back-link a { font-size:.76rem; color:var(--text-dim); text-decoration:none; cursor:pointer; }
...
.success-card h2 { font-size:1.4rem; font-weight:700; margin-bottom:6px; letter-spacing:-.02em; }
.success-card p { font-size:.85rem; color:var(--text-muted); line-height:1.6; }

/* Footer */
.card-footer-row { margin-top:18px; padding-top:16px; border-top:1px solid var(--border); display:flex; align-items:center; justify-content:space-between; }
...
@media(max-width:480px) { .auth-card{ padding:26px 18px 22px; } }
</style>
</head>
<body>
<div class="page-wrap">
<div class="auth-card">
  ...
  <div class="step active" id="step1">
    ...
    <div class="field">
      ...
      <div class="input-wrap">
        ...
        <input type="text" id="name" value="{{ old('name') }}" placeholder="John Doe" autocomplete="name"/>
      </div>
    ...
    <div class="field">
      ...
      <div class="input-wrap">
        <span class="input-icon">✉</span>
        <input type="email" id="reg_email" value="{{ old('email') }}" placeholder="you@example.com" autocomplete="email"/>
      </div>
    ...
    <div class="field">
      ...
      <div class="input-wrap">
        <span class="input-icon">🔑</span>
        <input type="password" id="pwd" placeholder="Min. 6 characters" oninput="checkStrength(this.value)"/>
        <span class="toggle-eye" onclick="togglePwd('pwd',this)">👁</span>
      </div>
      <div class="strength-wrap"><div class="strength-fill" id="sFill"></div></div>
      <div class="req-row">
    ...
    <div style="text-align:center">
      ...
      <div style="background:var(--surface-2);border:1px solid var(--border-lit);border-radius:10px;padding:14px 16px;text-align:left;margin-top:14px;">
        <ol style="padding-left:16px;margin:0;">
          <li style="font-size:.78rem;color:var(--text-muted);margin-bottom:5px;">Open your authenticator app and tap <strong style="color:var(--text)">+</strong></li>
          <li style="font-size:.78rem;color:var(--text-muted);margin-bottom:5px;">Select <strong style="color:var(--text)">Scan QR code</strong> or <strong style="color:var(--text)">Enter a setup key</strong></li>
          <li style="font-size:.78rem;color:var(--text-muted);">Paste the secret key above, or point camera at the QR</li>
        </ol>
      </div>
    </div>
  ...
  <div class="step" id="step3">
    ...
    <div style="background:var(--surface-2);border:1px solid var(--border-lit);border-radius:10px;padding:12px 14px;font-size:.78rem;color:var(--text-muted);line-height:1.55;margin-bottom:14px;">
...
let accessToken = '';   // filled after AJAX register
let wants2FA    = false;
let cd3Timer    = null;
...
async function submitRegister() {
  ...
  if (!name || !email || !pwd || !cpwd) { showAlert('Please fill in all fields.'); return; }
  if (pwd !== cpwd)   { showAlert('Passwords do not match.'); return; }
  if (pwd.length < 6) { showAlert('Password must be at least 6 characters.'); return; }
  if (!terms)         { showAlert('Please accept the Terms of Service.'); return; }
  hideAlert();
  ...
  try {
    const res = await fetch('{{ route("register.submit") }}', {
      ...
      body: JSON.stringify({ name, email, password: pwd, password_confirmation: cpwd }),
    });
...
async function submitVerify() {
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      setTimeout(() => { window.location.href = data.redirect ?? '/dashboard'; }, 2200);
    } else {
      ...
      btn.disabled = false; btn.textContent = '✓ Verify & Finish Setup';
    }
  } catch (err) {
    console.error(err);
    showAlert('Something went wrong. Please try again.');
...
document.querySelectorAll('.otp-box').forEach((b, i, all) => {
  ...
  b.addEventListener('keydown', e => {
    if (e.key === 'Backspace' && !b.value && i > 0) {
      all[i-1].value=''; all[i-1].classList.remove('filled'); all[i-1].focus();
    }
  });
  b.addEventListener('paste', e => {
    e.preventDefault();
    const d = (e.clipboardData||window.clipboardData).getData('text').replace(/\D/g,'').slice(0,6);
    [...d].forEach((ch,idx) => { if(all[idx]){ all[idx].value=ch; all[idx].classList.add('filled'); } });
...
// ═══════════════════════════════════════
function startTimer() {
  let t = 30; document.getElementById('cd3').textContent = t;
  clearInterval(cd3Timer);
...
function checkStrength(v) {
  const c = { 'r-len':v.length>=6, 'r-up':/[A-Z]/.test(v), 'r-num':/[0-9]/.test(v), 'r-sym':/[^A-Za-z0-9]/.test(v) };
...
function showAlert(m) { document.getElementById('alertMsg').textContent=m; document.getElementById('alertBox').classList.add('show'); window.scrollTo({top:0,behavior:'smooth'}); }
